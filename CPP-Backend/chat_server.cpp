...
void loadUsers() {
    ...
    while (std::getline(file, line)) {
        ...
        if (colon != std::string::npos) {
            User user;
            user.username = line.substr(0, colon);
            user.password = line.substr(colon + 1);
...
void loadMessages() {
    std::cout << "Loading messages from file...\n";
...
// string parseQueryParam(const std::string& query, const std::string& param) {
...
std::string handleRequest(const std::string& request) {
    ...
    if (request.find("GET /getusers") != std::string::npos) {
        ...
        std::cout << "Handling /getmessages request\n";
        ...
        std::cout << "Requested messages between: " << user1 << " and " << user2 << "\n";
        std::cout << "Total messages in memory: " << messages.size() << "\n";
        ...
        for (const auto& msg : messages) {
            if ((msg.sender == user1 && msg.receiver == user2) ||
                ...
                std::cout << "Sending message: " << msg.sender << "|" << msg.content << "|" << msg.timestamp << "\n";
            }
        }
        LeaveCriticalSection(&cs);
    }
